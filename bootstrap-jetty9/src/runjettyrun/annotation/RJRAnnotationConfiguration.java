...
public class RJRAnnotationConfiguration extends AnnotationConfiguration {
		...
		for (String path : rjrClasspaths) {
			...
			if (file.isDirectory()) {
				...
				if(folder.isDirectory()){
					// FIX for Jetty9
					parseClasses(context, file.toURI(), parser); //load annotation from classes file first
				}else{
     ...
     * @throws Exception
     */
    public void parseClasses (final WebAppContext context,URI classesDirUri , final AnnotationParser parser)
//    public void parseClasses (final WebAppContext context,Resource classesDir , final AnnotationParser parser)
    throws Exception
    {
    	...
    	FileResource classDir = new FileResource(classesDirUri.toURL());
        if (classDir.exists())
